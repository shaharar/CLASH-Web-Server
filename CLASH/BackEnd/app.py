...
def retrieve_features_by_category_for_download():
    ...
    result_df = retrieve_features_by_categories(feature_categories,full_search_results)
    print(result_df)
    # conn = pymssql.connect(server,user,password,database)
    # cursor = conn.cursor(as_dict = True)
    ...
    #     result_df = pd.merge(search_results,features_df,how='inner',on='mirTar_id')
    jsonResult = result_df.to_json(orient='records')
    # conn.close()
    return jsonResult

def retrieve_features_by_categories(feature_categories, results):
    mirtar_id_list = results['mirTar_id'].tolist()
    # print(feature_categories)
    if(feature_categories is None or results.empty):
        # jsonResult = search_results.to_json(orient='records')
        result_df = results
    elif(~results.empty):
        dfs = []
        ...
        features_df = functools.reduce(lambda df1,df2: pd.merge(df1,df2,how='inner',on='mirTar_id'), dfs)
        # print(results)
        result_df = pd.merge(results,features_df,how='inner',on='mirTar_id')
        conn.close()
    return result_df
...
def retrieve_info_by_filter_inputs():
    ...
    filtered_search_results = intersect_results
...
def retrieve_search_results():
    ...
    search_results = df[['mirTar_id']]
    # print(search_results)
    global full_search_results
    full_search_results = df
    # return df
